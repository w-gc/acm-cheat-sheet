...
\begin{NoteCode}
template<class T>
Node<T>* build_post_in(const vector<T> &pvec, const vector<T> &ivec, int len, int pb, int ib){
    // 后序pvec[pb, pb+1, ...., pb+len-1]
    // 中序ivec[ib, ib+1, ...., ib+len-1]
    Node<T> *root = new Node<T>(pvec[pb + len - 1]);
    if(len == 1) return root;

    int new_ib = ib;
    while( new_ib < ib + len && ivec[new_ib] != root->val) ++new_ib;
    if(new_ib == len) return nullptr;
    
    int left_len = new_ib - ib;
    
    // 后序pvec[pb, ...., pb+left_len-1]
    // 中序ivec[ib, ib+1, ...., ib+left_len-1]
    root->left = build_post_in(pvec, ivec, left_len, pb, ib);

    // 后序pvec[pb+left_len, pb+left_len+2, ...., pb+len - 2]
    // 中序ivec[new_ib+1, new_ib+2, ...., new_ib+right_len]
    root->right = build_post_in(pvec, ivec, len - left_len - 1, pb + left_len, new_ib + 1);

    return root;
}
// vector<int> in_v = {4, 2, 5, 1, 6, 3, 7};
// vector<int> post_v = {4, 5, 2, 6, 7, 3, 1};
// Node<int> *root = build_post_in(post_v, in_v, post_v.size(), 0, 0);
\end{NoteCode}
...
template<class heap_elems_type>
class Heap{
public:
    typedef heap_elems_type value_type;
...
public:
    void set_cmp(bool (*cmp)(const_iterator, const_iterator));
    bool empty();
    size_type size(); 
    void sift_down(const int start);
    void sift_up(const int start);
    void push(const value_type x);
    void pop();
    value_type top();

    Heap(const int capacity, bool (*cmp)(const_iterator, const_iterator) );
    ~Heap();
private:
    value_type *_elems;
    size_type _size;
    size_type _capacity;
    bool (*_cmp)(const_iterator, const_iterator);
};

template<class T>
void Heap<T>::set_cmp(bool (*cmp)(const_iterator a, const_iterator b) ){
    return this->_elems = cmp;
}

template<class T>
Heap<T>::Heap(const int capacity, bool (*cmp)(const_iterator a, const_iterator b))
: _size(0), _capacity(capacity), _elems(new value_type[capacity]), _cmp(cmp) { }

template<class T>
Heap<T>::~Heap(){ delete _elems; }

template<class T>
bool Heap<T>::empty(){ return _size == 0; }

template<class T>
typename Heap<T>::size_type Heap<T>::size(){ return _size; }

template<class T>
void Heap<T>::sift_down(const int start){
    int i = start;
    const value_type tmp = _elems[start];
    for(int j = 2 * i + 1; j < _size; j = 2 * j + 1){
        // j指向两子女中小者
        if( j < (_size - 1) && _cmp(&_elems[j], &_elems[j+1]) ) j++;

        if( !_cmp(&tmp, &_elems[j]) ) break;
        else{
            // tmp >= _elems[j], 数据_elems[j]上移
            _elems[i] = _elems[j];
            i = j;
        }
    }
...
template<class T>
void Heap<T>::sift_up(const int start){
    int j = start;
    int i = (j - 1) / 2; // j的父节点
    const value_type tmp = _elems[start];
    while (j > 0){
        if( ! _cmp(&_elems[i], &tmp)) break;
        else{
            // _elems[i] >= tmp, 数据_elems[i]下移
            _elems[j] = _elems[i];
            j = i;
            i = (i - 1) / 2;
        }
...
template<class T>
void Heap<T>::push(const value_type x){
    if(_size == _capacity){
        // TODO
    }
    _elems[_size] = x;
...
template<class T>
typename Heap<T>::value_type Heap<T>::top(){
    if(_size <= 0){
        // TODO
    }
    return _elems[0];
